...
//        //recaptcha validation end
    ...
    $query->where('phone', $request->user_id)
        ...
        if (isset($user) && auth('customer')->attempt(['email' => $user['email'], 'password' => $request['password']], $remember)) {
            ...
            // Trigger SSO login to account folder - MUST succeed for login to complete
    ...
    public function logout(Request $request)
    {
		...
		if ($userId) {
			$ssoService = app(SSOService::class);
			$ssoResult = $ssoService->syncLogout($userId);
			
			if ($ssoResult['success']) {
				// Redirect to account folder logout to complete SSO logout
				return redirect($ssoResult['url']);
			}
		}
