...
class ForgotPasswordController extends Controller
{
    ...
    public function resetPasswordRequest(Request $request): RedirectResponse|JsonResponse
    {
        ...
        // If identity is an email, handle email reset flow immediately
        ...
        // Username-based recovery: if not an email and not phone-like, try username and send email link to the account's email
        $isPhoneLike = (bool) $this->getLast10Digits($identity);
        if (!$isPhoneLike) {
            $customer = $this->customerRepo->getByIdentity(filters: ['identity' => $identity]);
            if ($customer) {
                if ($customer->is_active == 0) {
                ...
                if (empty($customer['email'])) {
                    Toastr::error(translate('No_email_found_for_this_account'));
                    return back();
                }
                try {
                    $token = Str::random(120);
        ...
        // Otherwise, treat identity as phone and continue with OTP flow
        ...
        $last10Digits = $this->getLast10Digits($identity);
        ...
        $customer = $this->customerRepo->getByIdentity(filters: ['phone_last_digits' => $last10Digits]);
        ...
        $normalizedPhone = '+92' . $last10Digits;
        ...
        if (isset($OTPVerificationData) && Carbon::parse($OTPVerificationData->created_at)->diffInSeconds() < $otpIntervalTime) {
            ...
            if ($verificationBy == 'phone' && $firebaseOTPVerification && $firebaseOTPVerification['status']) {
                ...
                $firebaseResponse = $this->firebaseService->sendOtp($normalizedPhone);
    ...
    public function resetPasswordSubmit(Request $request): View|Redirector|RedirectResponse
    {
        ...
        if (isset($data) && $customer) {
            $hashedPassword = bcrypt(str_replace(' ', '', $request['password']));
	...
	/**
 * Extract last 10 digits from any phone number input
 */
private function getLast10Digits(string $phone): ?string
{
