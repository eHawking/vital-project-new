...
class DashboardController extends BaseController
{
    ...
    public function getView(): View
    {
		...
		$totalDelivered = Order::where('seller_id', $vendorId)
        ...
        $shopOrders = DB::table('shop_orders')
            ->where('ordered_by', $vendorId)
            ->where('payment_status', 'paid')
            ->where('status', 'approved')
            ->pluck('order_amount');

        $franchiseOrders = DB::table('franchise_orders')
            ->where('ordered_by', $vendorId)
            ->where('payment_status', 'paid')
            ->where('status', 'approved')
            ->pluck('order_amount');

        $totalInHouseDelivered = $shopOrders->sum() + $franchiseOrders->sum();
        $totalInHouseOrdersDelivered = $shopOrders->count() + $franchiseOrders->count();
		
		
        $totalOrdersDelivered = Order::where('seller_id', $vendorId)
            ->where('customer_type', 'customer')
            ->where('order_status', 'delivered')
            ->where('order_type', 'POS')
            ->count();
        ...
        $dashboardData = [
            ...
            'totalInHouseDelivered' => $totalInHouseDelivered,
            'totalOrdersDelivered' => $totalOrdersDelivered,
            'totalInHouseOrdersDelivered' => $totalInHouseOrdersDelivered,
        ];
