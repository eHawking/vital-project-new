...
class HomeController extends Controller
{
    ...
    {
        // Check for session expired message from account folder
        if (request()->has('session_expired') && request()->has('message')) {
    ...
    public function default_theme(): View
    {
        ...
        $topVendorsListQuery = $this->cacheHomePageTopVendorsList()->filter(function ($shop) {
            return optional($shop->seller)->vendor_type !== 'franchise' && optional($shop->seller)->vendor_type !== 'shop';
        });
        $topVendorsList = ProductManager::getPriorityWiseTopVendorQuery($topVendorsListQuery);
    ...
    public function theme_aster(): View
    {
        ...
        $topVendorsList = ProductManager::getPriorityWiseTopVendorQuery(query: $topVendorsListQuery);
        $clearanceSaleProducts = $this->cacheHomePageClearanceSaleProducts();
    ...
    public function theme_fashion(): View
    {
        ...
        $vendorList = $this->cacheShopTable()->filter(function ($shop) {
            return optional($shop->seller)->vendor_type !== 'franchise' && optional($shop->seller)->vendor_type !== 'shop';
        });
