...
public function getVariantPrice(Request $request): array
{
    ...
    // Add this line to check if the user is a guest
    ...
    if ($string != null && isset($product->variation) && !empty(json_decode($product->variation))) {
        $count = count(json_decode($product->variation));
        for ($i = 0; $i < $count; $i++) {
            $variation_data = json_decode($product->variation)[$i];
            if ($variation_data->type == $string) {
                // MODIFICATION: Check for guest and use guest_price if available for variations
                $base_price = ($is_guest && isset($variation_data->guest_price)) ? $variation_data->guest_price : $variation_data->price;
                $unit_price = $variation_data->price; // Original price for reference
                $quantity = $variation_data->qty;
            }
        }
    } else {
        // MODIFICATION: Check for guest and use guest_price for simple products
        $base_price = ($is_guest && isset($product->guest_price)) ? $product->guest_price : $product->unit_price;
    ...
    if ($product['product_type'] == 'digital' && $digitalVariation) {
        // MODIFICATION: Check for guest and use guest_price for digital products
        $base_price = ($is_guest && isset($digitalVariation->guest_price)) ? $digitalVariation->guest_price : $digitalVariation['price'];
        $unit_price = $digitalVariation['price'];
        $quantity = 100; // Digital products might not have stock qty

        foreach ($getCartList as $cartItem) {
            if ($cartItem['variant'] == $request['variant_key']) {
                $string = $cartItem['variant'];
            }
        }
    }

    // All calculations should now use the correct base_price
    if (isset($base_price)) {
        $tax = $product->tax_model == 'exclude' ? Helpers::tax_calculation(product: $product, price: $base_price, tax: $product['tax'], tax_type: $product['tax_type']) : 0;
        $update_tax = $tax * $requestQuantity;
        $discount = getProductPriceByType(product: $product, type: 'discounted_amount', result: 'value', price: $base_price);
        $price = $base_price - $discount + $tax;
        $discountedUnitPrice = $base_price - $discount;
    }
    ...
    return [
        ...
        'update_tax' => $product['tax_model'] == 'exclude' ? webCurrencyConverter($update_tax) : 'incl.',
        ...
        'unit_price' => webCurrencyConverter($price),
        'total_unit_price' => webCurrencyConverter($unit_price),
