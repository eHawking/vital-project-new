...
class ProductDetailsController extends Controller
{
    ...
    if (auth('customer')->check()) {
        ...
        $mainUser = \DB::connection('mysql2')->table('users')
            ->where('username', $username)
            ->first();

        if ($mainUser) {
            $plan = ($mainUser->plan_id == 1 || $mainUser->is_btp == 1) ? 1 : 0;
            $isBtp = $mainUser->is_btp ?? 0;
            $isShare = $mainUser->is_share ?? 0;

            $dsp = \DB::connection('mysql2')->table('users')
                ->where('ref_by', $mainUser->id)
                ->where('username', 'like', '%dsp%')
                ->get();
        }
    }
            ...
            return view(VIEW_FILE_NAMES['products_details'], compact('product', 'wishlistStatus', 'countWishlist',
                ...
                'sellerTemporaryClose', 'inHouseVacationStartDate', 'inHouseVacationEndDate', 'inHouseVacationStatus', 'inHouseTemporaryClose', 'positiveReview', 'overallRating', 'decimalPointSettings', 'moreProductFromSeller', 'productsForReview', 'productsCount', 'totalReviews', 'rating', 'productReviews', 'avgRating', 'topRatedShops', 'newSellers', 'deliveryInfo', 'productsTopRated', 'productsThisStoreTopRated', 'previewFileInfo', 'productAuthorsInfo', 'productPublishingHouseInfo', 'firstVariationQuantity', 'productDetailsMeta', 'plan', 'isBtp', 'isShare', 'dsp'));
        }
