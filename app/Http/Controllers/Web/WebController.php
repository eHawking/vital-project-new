...
class WebController extends Controller
{
    ...
    public function search_shop(Request $request)
    {
        ...
        $sellers = Shop::where(function ($q) use ($key) {
            ...
            return $query->where(['status' => 'approved'])->whereNotIn('vendor_type', ['franchise', 'shop']);
        })->paginate(30);
...
$vendorsList = Shop::active()
            ->whereHas('seller', function ($q) {
                $q->whereNotIn('vendor_type', ['franchise', 'shop']);
            })
    ...
    public function checkout_details(Request $request)
    {
        ...
        $zipRestrictStatus = false; // MODIFICATION: Zip code removed
        $countries = $countryRestrictStatus ? $this->get_delivery_country_array() : COUNTRIES;
        $zipCodes = []; // MODIFICATION: Zip code removed
        ...
        return view(VIEW_FILE_NAMES['order_shipping'], [
            'physical_product_view' => $hasPhysicalProduct,
            'hasPhysicalProduct' => $hasPhysicalProduct,
            'hasDigitalProduct' => $hasDigitalProduct,
            'zip_codes' => $zipCodes,
    ...
    {
        // BTP COD Check
        $cartItems = Cart::whereIn('cart_group_id', CartManager::get_cart_group_ids(request: $request, type: 'checked'))
                        ->where(['is_checked' => 1])
                        ->with('product')
                        ->get();
        ...
        foreach ($cartItems as $cartItem) {
            if (isset($cartItem->product) && $cartItem->product->is_btp == 1) {
                ...
                $errorMessage = 'Cash on Delivery is not allowed for the product: ' . $productName . '. You need to deposit in your wallet and then pay through the wallet.';
    ...
    private function handleBtpStatusUpdate(array $orderIds): void
    {
        $customer = auth('customer')->user();

        if (!$customer || !isset($customer->username)) {
            return;
        }

        $order = Order::find($orderIds[0] ?? 0);

        if ($order && $order->payment_status == 'paid') {
            $orderDetails = OrderDetail::with('product')->whereIn('order_id', $orderIds)->get();

            $containsBtpProduct = $orderDetails->contains(function ($detail) {
                return isset($detail->product) && $detail->product->is_btp == 1;
            });

            if ($containsBtpProduct) {
                try {
                    $secondaryUser = SecondaryUser::where('username', $customer->username)->first();

                    if ($secondaryUser && $secondaryUser->is_btp != 1) {
                        $secondaryUser->is_btp = 1;
                        $secondaryUser->save();
                    }
                } catch (\Exception $e) {
                    Log::error('BTP Update Failed for user: ' . $customer->username . ' - ' . $e->getMessage());
                }
            }
        }
    }
    ...
    public function getOrderPlaceView(Request $request): View
    {
        ...
        if ($orderIds) {
            $this->handleBtpStatusUpdate($orderIds);
        }
    ...
    public function checkout_complete_wallet(Request $request = null)
    {
        ...
        if ($paymentAmount > $user->wallet_balance) {
            ...
            $this->handleBtpStatusUpdate($order_ids);
