    ...
    public function getByIdentity(array $filters = []): ?Model
{
    return $this->user
        ...
        ->when(isset($filters['identity']) && $filters['identity'], function ($query) use ($filters) {
            return $query->orWhere(function ($query) use ($filters) {
                return $query->whereNotNull('username')->where(['username' => $filters['identity']]);
            });
        })
		->when(isset($filters['phone_last_digits']) && $filters['phone_last_digits'], function ($query) use ($filters) {
            return $query->where('phone', 'like', '%' . $filters['phone_last_digits']);
        })
        ->first();
}
        ...
        $query = $this->user->with($relations)
            ...
            ->when($searchValue, function ($query) use ($searchValue) {
                $query->orWhere('f_name', 'like', "%$searchValue%")
					...
					->orWhere(DB::raw("CONCAT(f_name, ' ', l_name)"), '=', $searchValue)
                    ->orWhere('username', '=', $searchValue)
					->orWhere('city', '=', $searchValue)
                ...
                $query->where(function ($query) use ($searchValue) {
                    foreach ($searchValue as $value) {
                        $query->orWhere('f_name', 'like', "%$value%")
							...
							->orWhere(DB::raw("CONCAT(f_name, ' ', l_name)"), '=', $value)
