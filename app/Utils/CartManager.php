    ...
    public static function addToCartPhysicalProduct($request, $product, $shippingType, $sellerShippingList): array
{
    ...
    $user = Helpers::getCustomerInformation($request);
    $is_guest = ($user == 'offline'); // Check for guest
    ...
    if ($string != null && isset($product->variation) && !empty(json_decode($product->variation))) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($variation_data->type == $string) {
                // MODIFICATION: Set price based on user type for variations
                $price = ($is_guest && isset($variation_data->guest_price)) ? $variation_data->guest_price : $variation_data->price;
                if ($variation_data->qty < $request['quantity']) {
                    return ['status' => 0, 'message' => translate('out_of_stock!')];
                }
            }
        }
    } else {
        // MODIFICATION: Set price based on user type for simple products
        $price = ($is_guest && isset($product->guest_price)) ? $product->guest_price : $product->unit_price;
    }
    ...
    if ($cart) {
        ...
        if ($request['buy_now'] == 1) {
            ...
            if ($product['product_type'] == 'physical' && $shippingType == 'order_wise') {
                ...
                return [
                    'status' => 0,
                    'message' => translate('Shipping_Not_Available_for_this_Shop'),
                ];
