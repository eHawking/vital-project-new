...
class ProductManager
{
    ...
    public static function updateProductPriceInCartList(object|array $request): void
    {
        ...
        foreach ($cartItemsList as $cartItem) {
            ...
            if (!$cartItemProduct) continue;

            if (empty($cartItem->variant)) {
                
                $correctPrice = $cartItemProduct->unit_price;
                if ($user == 'offline' && isset($cartItemProduct->guest_price) && $cartItemProduct->guest_price > 0) {
                    $correctPrice = $cartItemProduct->guest_price;
                }

                
                if ($cartItem->price != $correctPrice) {
                    Cart::where(['id' => $cartItem['id']])->update(['price' => $correctPrice]);
                }
