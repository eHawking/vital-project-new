...
$(".action-category-filter").on("change", (event) => {
    ...
    getUrl.searchParams.set("category_id", categoryId);
    getUrl.searchParams.set("page", 1); // Reset to first page
    
    // Load products via AJAX
    loadProducts(getUrl.toString());
});
...
function loadProducts(url) {
    ...
    // Add ajax parameter to URL if not present
    let separator = url.includes('?') ? '&' : '?';
    if (!url.includes('ajax=')) {
        url = url + separator + 'ajax=1';
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response && response.success && response.html) {
                ...
                // Count current products before update
                let oldCount = $('#product-list-container .pos-item-wrap .pos-product-item').length;
                console.log('Old products count:', oldCount);
                
                // Update DOM
                $('#product-list-container').html(response.html);
                
                // Count new products after update
                let newCount = $('#product-list-container .pos-item-wrap .pos-product-item').length;
                console.log('New products count:', newCount);
                console.log('Products updated in DOM');
                ...
                let cleanUrl = url.replace('&ajax=1', '').replace('?ajax=1', '');
                ...
                if (typeof toastr !== 'undefined') {
                    toastr.error(response.error || 'Invalid response format');
                }
            }
...
// AJAX Pagination Handler - Use event delegation on document
...
// Also prevent any default link behavior on pagination wrapper
$(document).on('click', '.pos-pagination-wrapper a', function(e) {
...
function basicFunctionalityForCartSummary() {
    ...
    $(".action-form-submit").on("click", function () {
        ...
        if (submitBtn.prop('disabled') || submitBtn.hasClass('processing')) {
            return false;
        }
                ...
                if (result.value) {
                    ...
                    $.post({
                        ...
                        error: function (xhr, status, error) {
                            ...
                            // Show error message
                            if (typeof toastr !== 'undefined') {
                                toastr.error('Order placement failed. Please try again.');
                            } else if (typeof toastMagic !== 'undefined') {
                                toastMagic.error('Order placement failed. Please try again.');
                            } else {
...
function addToCart(form_id = "add-to-cart-form") {
    if (checkAddToCartValidity()) {
        ...
        $.post({
            ...
            success: function (data) {
                if (data.data == 1) { // This block handles a successful CART UPDATE.
                    $('#quick-view').modal('hide'); // <-- CHANGE: Modal is now closed on update.
                    $("#cart-summary").empty().html(data.view);
                    ...
                    removeFromCart();
                    basicFunctionalityForCartSummary();
                    return false; // We can keep this return as all necessary actions are complete.
                } else if (data.data == 0) { // This block handles an OUT OF STOCK error.
                    Swal.fire({
                        icon: "error",
                        title: $("#message-cart-word").data("text"),
                ...
                // This entire section below handles a successful ADD TO CART.
                $('#quick-view').modal('hide'); // <-- CHANGE: Replaced click() with a direct method call.

                if (data.data != 0) { // This condition handles other success scenarios.
...
$("#customer-search").on("keyup", function (e) {
    ...
    // Only search when Enter is pressed or after a short delay
...
function performCustomerSearch(searchTerm) {
    $.get({
        url: $("#route-vendor-pos-search-customer").data("url"),
        dataType: "json",
        data: {
            search: searchTerm,
        },
        beforeSend: function () {
            $("#customer-search-content").html('<div class="text-center p-2"><div class="spinner-border spinner-border-sm" role="status"></div></div>');
        ...
        success: function (data) {
            if (data.success) {
                ...
                let html = `
                    <div class="customer-result-item p-2 cursor-pointer hover-bg-light" 
                ...
                `;
                $("#customer-search-content").html(html);
                
                // Attach click event to select customer
                $(".customer-result-item").on("click", function() {
                    selectCustomer(
                        $(this).data("customer-id"),
                        $(this).data("customer-name"),
                        $(this).data("customer-username"),
                        $(this).data("customer-phone")
                    );
                });
            } else {
                $("#customer-search-content").html(`
                    <div class="text-center p-2 text-danger">
                        <i class="tio-clear-circle-outlined"></i> ${data.message}
                    </div>
                `);
            }
        },
...
function selectCustomer(customerId, customerName, customerUsername, customerPhone) {
    ...
    $("#customer-display-name").text(customerName);
    $("#customer-display-details").text(`@${customerUsername} | ${customerPhone}`);
    $("#selected-customer-info").show();
    $("#customer-search").val("");
    $("#customer-search-results").hide();
    
    // Change customer in POS - using POST like the original dropdown implementation
    $.post({
        url: $("#route-vendor-pos-change-customer").data("url"),
        data: {
            _token: $('meta[name="_token"]').attr("content"),
...
$("#clear-customer").on("click", function() {
    ...
    if (typeof toastr !== 'undefined') {
        ...
        alert("Customer cannot be cleared. Only registered customers can place orders. Please complete this order or start a new order.");
    }
});
