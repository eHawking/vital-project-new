...
if (auth('customer')->check()) {
    ...
    $mainUser = \DB::connection('mysql2')->table('users')->where('username', $username)->first();

    if ($mainUser) {
        if ($mainUser->plan_id == 1 || $mainUser->is_btp == 1) {
            $plan = 1;
        }

        $isBtp = $mainUser->is_btp ?? 0;
        $isShare = $mainUser->is_share ?? 0;
    }
}
?>

@if(auth('customer')->check() && ($plan == 1 || $isBtp == 1 || $isShare == 1))
      <div class="share--icons text-align-direction">
    ...
    <div class="text-center mb-2">
        <div class="row">
            <div class="col-6 col-md-6">
                <div class="card custom-card">
                    <div class="card-body custom-card-body">
                        <img width="30" src="{{ theme_asset('public/assets/front-end/img/icons/loyalty-point.webp') }}" class="dark-support mobile-img" alt="">
            ...
            <div class="col-6 col-md-6">
                <div class="card custom-card">
                    <div class="card-body custom-card-body">
                        <img width="30" src="{{ theme_asset('public/assets/front-end/img/icons/profile-icon5.webp') }}" class="dark-support mobile-img" alt="">
...
@else
    <h3 class="font-weight-normal text-accent d-flex align-items-end gap-2 pt-1">
        <span class="discounted-unit-price fs-24 font-bold">
            {{ webCurrencyConverter(amount: $product->guest_price ?? $product->unit_price) }}
        </span>
    </h3>
@endif
                                    </div>
...
<script>
   document.addEventListener('DOMContentLoaded', function () {
    const shareLinks = document.querySelectorAll('.share-on-social-media');

    shareLinks.forEach(link => {
        link.addEventListener('click', function (e) {
            e.preventDefault();

            const productUrl = encodeURIComponent(this.dataset.action);
            const socialMedia = this.dataset.socialMediaName;

            let shareUrl = '';

            // Build the correct share URL based on the social media
            if (socialMedia.includes('facebook')) {
                ...
                shareUrl = `https://twitter.com/intent/tweet?text= ${text}${productUrl}`;
    ...
    // Toggle the share dropdown (optional)
    const shareButton = document.querySelector('.product-share-icons > a');
    const shareList = document.querySelector('.product-share-icons ul');
    ...
    if (shareButton && shareList) {
        ...
        document.addEventListener('click', function (e) {
            if (!shareButton.contains(e.target) && !shareList.contains(e.target)) {
