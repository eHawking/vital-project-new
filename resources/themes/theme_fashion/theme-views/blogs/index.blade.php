    ...
    <div class="blog-root-container" data-platform="{{ isset($blogPlatform) && $blogPlatform == 'app' ? 'app' : 'web' }}" data-theme="{{ request('theme', 'light') }}">
        <div class="container">
            <div class="rounded-10 my-4 text-center d-none d-sm-block position-relative blog-banner-container">
                <div class="text--primary position-absolute w-100">
            ...
            <div class="d-block d-sm-none">
                <h1 class="fs-16 fw-semibold my-3 text-center">
                    {{ $blogTitle != '' ? $blogTitle : translate('Blog') }}
                </h1>
            </div>
            ...
            @if($blogList->total() > 0 || (request()->has('search') || request()->has('category') || request()->has('write')))
                <div class="row g-4 sticky-top-nav-search">
                    <div class="col-lg-8 order-1 order-lg-0">
                        <div class="position-relative">
                            <ul class="blog-top-nav d-flex gap-3">
                                <li class="{{ request('category') == '' ? 'active' : ''}}">
                                    <a href="{{ isset($blogPlatform) && $blogPlatform == 'app' ? route('app.blog.index', ['locale' => request('locale'), 'theme' => request('theme')]) : route('frontend.blog.index') }}" class="border border-primary-light rounded-10 px-3 py-2 d-flex align-items-center h-45px">
                                ...
                                @foreach($blogCategoryList as $blogCategory)
                                    <li class="{{ request('category') == $blogCategory?->name ? 'active' : ''}}">
                                        @if(isset($blogPlatform) && $blogPlatform == 'app')
                                            <a href="{{ route('app.blog.index', ['category' => $blogCategory?->name, 'locale' => request('locale'), 'theme' => request('theme')]) }}" class="border border-primary-light rounded-10 px-3 py-2 d-flex align-items-center h-45px">
                                                <span class="opacity-60">{{ Str::limit($blogCategory->name, 25) }}</span>
                                            </a>
                                        @else
                                            <a href="{{ route('frontend.blog.index', ['category' => $blogCategory?->name]) }}" class="border border-primary-light rounded-10 px-3 py-2 d-flex align-items-center h-45px">
                                                <span class="opacity-60">{{ Str::limit($blogCategory->name, 25) }}</span>
                                            </a>
                                        @endif
                                    </li>
                            ...
                            <div class="blog-top-nav_prev-btn justify-content-center align-items-center">
                            ...
                            <div class="blog-top-nav_next-btn justify-content-center align-items-center">
                        ...
                        @if(request('search') || request('category') || request('write'))
                            <div class="my-2">
                        ...
                        <div class="">
                            <form action="{{ isset($blogPlatform) && $blogPlatform == 'app' ? route('app.blog.index', ['locale' => request('locale'), 'theme' => request('theme')]) : route('frontend.blog.index') }}" method="get" id="search-form">
                                ...
                                <div class="input-group-overlay input-group-sm position-relative">
                                    ...
                                    <input placeholder="{{ translate('Search_Blog') }}" value="{{ request('search') }}"
                                           class="__inline-38 cz-filter-search form-control form-control-sm appended-form-control h-45px bg-absolute-white form-control-dark-mode"
                            ...
                            @if(request('search'))
                                <div class="my-2 d-flex gap-2 align-items-baseline justify-content-end clear-all-search cursor-pointer">
                                    <h6>{{ translate('Clear_All_Search') }}</h6>
                                    <button type="button" class="btn border-0 fs-14 fw-bold lh-1 m-0 p-0 title-color"><i class="bi bi-x-lg fw-bold"></i></button>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="row g-4 mb-lg-5 pb-lg-4">
                ...
                @elseif($blogList->total() <= 0)
                    ...
                    <div class="col-lg-4">
                        <div class="sticky-top-wrapper top-170px">
                            <div class="card border-light-base mb-4">
                                            ...
                                            @if ($recentBlogIndex < 6)
                                                <div class="recent-post">
                                                    <div class="d-flex gap-3 align-items-center">
                                                        <img class="h-80px aspect-1 object-cover rounded-10 object-fit-cover"
                ...
                @else
                    <div class="col-lg-8 pt-3">
                        <div class="row g-4 mb-0 mb-lg-5">
                            ...
                            @if(count($blogList) > 0)
                                <div class="col-12">
                                    <div id="paginator-ajax" class="blog-paginator">
                                        {!! $blogList->links() !!}
                                    </div>
                                </div>
                            @endif
