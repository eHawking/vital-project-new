...
>
    ...
    <div class="p-3 d-flex flex-column gap-12">
        @if(isset($blogPlatform) && $blogPlatform == 'app')
            <h3 class="mb-0 fw-semibold letter-spacing-0 line-clamp-2">
            ...
            @if($blogItem?->category?->name)
                <a href="{{ route('app.blog.index', ['category' => $blogItem?->category?->name, 'locale' => request('locale'), 'theme' => request('theme')]) }}" title="{{ $blogItem?->category?->name }}" class="title-color border rounded-30 px-3 py-1 mb-0 fs-12-mobile line-clamp-1 w-fit-content">
                    <span class="opacity-60">{{ Str::limit($blogItem?->category?->name ?? translate('Uncategorized'), 25)  }}</span>
                </a>
            @endif
        @else
            <h3 class="mb-0 fw-semibold letter-spacing-0 line-clamp-2">
            ...
            @if($blogItem?->category?->name)
                <a href="{{ route('frontend.blog.index', ['category' => $blogItem?->category?->name]) }}" title="{{ $blogItem?->category?->name }}" class="title-color border rounded-30 px-3 py-1 mb-0 fs-12-mobile line-clamp-1 w-fit-content custom-link-item">
                    <span class="opacity-60">{{ Str::limit($blogItem?->category?->name ?? translate('Uncategorized'), 25)  }}</span>
                </a>
            @endif
        @endif
                ...
                <div class="d-flex justify-content-between align-content-center gap-3">
                    @if(isset($blogPlatform) && $blogPlatform == 'app')
                        <span class="opacity-80 fs-14 d-flex gap-1 fs-12-mobile text-nowrap">
                            ...
                            <a href="{{ route('app.blog.index', ['writer' => $blogItem?->writer, 'locale' => request('locale'), 'theme' => request('theme')]) }}" class="title-color fw-semibold max-width-20ch line-clamp-1 custom-link-item"
                    ...
                    @else
                        <span class="opacity-80 fs-14 d-flex gap-1 fs-12-mobile text-nowrap">
                        {{ $blogItem?->writer ? translate('By') : '' }}
                            <a href="{{ route('frontend.blog.index', ['writer' => $blogItem?->writer]) }}" class="title-color fw-semibold max-width-20ch line-clamp-1 fs-12-mobile custom-link-item"
                               title="{{ $blogItem?->writer ?? '' }}">
                                {{ Str::limit($blogItem?->writer ?? '', 40, '...') }}
                            </a>
                        </span>
