<?php
    ...
    $googleStoreLink = getWebConfig(name: 'download_app_google_stroe') ?? [];
    $appleStoreLink = getWebConfig(name: 'download_app_apple_stroe') ?? [];
    $downloadSectionData = [
...
@if($cardShowAble && isset($blogPlatform) && $blogPlatform == 'web')
    <div class="download-user-app text-center" data-bg-img="{{ isset($downloadSectionData['app_background']['path']) ? $downloadSectionData['app_background']['path'] : theme_asset(path: 'assets/img/blogs/download-user-app-bg.png') }}">
        <div class="download-user-main">
            @isset($downloadSectionData['app_icon']['path'])
                <div class="mb-3">
                    <img width="60" height="60" src="{{ $downloadSectionData['app_icon']['path'] }}" alt="" class="object-fit-cover rounded">
                </div>
            @endisset
            <h3 class="mb-12px text-absolute-white fs-22 fs-20-mobile">{{ $downloadSectionData['title'] }}</h3>
            <p class="mb-60 px-2 text-absolute-white fs-16 fs-14-mobile">{!! $downloadSectionData['subtitle'] !!}</p>
            <div class="d-flex justify-content-between flex-wrap gap-3 mb-3">

                @if($downloadSectionData['google_app_button'])
                    <a href="{{ $downloadSectionData['google_app_button'] }}" target="_blank" class="rounded-10 bg-white px-3 py-2 d-flex justify-content-center align-items-center gap-3 flex-grow-1 text-black">
                ...
                @if($downloadSectionData['apple_app_button'])
                    <a href="{{ $downloadSectionData['apple_app_button'] }}" target="_blank"
                       class="rounded-10 bg-white px-3 py-2 d-flex justify-content-center align-items-center gap-3 flex-grow-1 text-black">
