<form class="search-form category-search-form" action="{{route('products')}}" type="submit">
    <div class="input-group search_input_group">
        <select class="select2-init" id="search_category_value_web" name="category_ids[]">
            ...
            <option value="{{ $category->id }}" {{ in_array($category->id, request('category_ids') ?? []) ? 'selected':'' }}>{{$category['name']}}</option>
            @endforeach
        </select>
