...
.product-share-icons ul {
    inset-inline-start: 3.5px;
        ...
        .custom-card {
            ...
            box-shadow: 3px 4px 0px rgb(126, 42, 252);
        }

        .custom-card:first-of-type {
            box-shadow: -3px 4px 0px rgb(126, 42, 252);
        }
...
/* Styles for Web (screens wider than 768px) */
                    ...
                    <div class="product-single-wrapper">
                        ...
                        @if($product->images!=null && json_decode($product->images)>0)
                            <div class="product-single-thumb">
                                ...
                                @else
                                    <div class="overflow-hidden rounded position-relative">
										@if(auth('customer')->check() && ($plan == 1 || $isBtp == 1 || $isShare == 1))
    <div class="product-share-icons">
        <a href="javascript:" class="share-icon" title="{{ translate('share') }}">
            <i class="bi bi-share-fill fs-24"></i>
        </a>
        <ul>
            @foreach(['facebook', 'twitter', 'linkedin', 'whatsapp'] as $social)
                <li>
                    <a href="javascript:" class="social_share_function"
                       data-social="{{ $social }}"
                       data-url="{{ route('product', $product->slug) }}">
                        <i class="bi bi-{{ $social }} fs-24"></i>
                    </a>
                </li>
            @endforeach
        </ul>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <input type="hidden" id="selectedSocial">
                    <label>Select one of your DSPs to generate a link. If no DSP is selected, the link will be generated using your DDS ID.</label>
                    <select class="form-control" id="selectDSP">
                        <option value="">Select DSP</option>
                        @foreach($dsp as $user)
                            <option value="{{$user->username}}">{{$user->username}}</option>
                        @endforeach
                    </select>
                    <label class="mt-2">Select Position:</label>
                    <select class="form-control" id="selectPos">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="shareButton" disabled>Share</button>
                </div>
            </div>
        </div>
    </div>

    <input type="hidden" id="username" value="{{ auth('customer')->user()->username }}">
    ...
    <div class="text-center mb-4">
        <div class="row">
            <div class="col-6 col-md-6">
                <div class="card custom-card">
                    <div class="card-body custom-card-body">
                        <img width="30" src="{{ theme_asset('assets/img/icons/loyalty-point.webp') }}" class="dark-support mobile-img" alt="">
                        <span class="web-text"><strong>{{ $product->pv }} PV</strong></span>
                        <span class="mobile-text"><strong>{{ $product->pv }} PV</strong></span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-6 col-md-6">
                <div class="card custom-card">
                    <div class="card-body custom-card-body">
                        <img width="30" src="{{ theme_asset('assets/img/icons/profile-icon5.webp') }}" class="dark-support mobile-img" alt="">
                        <span class="web-text"><strong>{{ $product->bv }} BV</strong></span>
                        <span class="mobile-text"><strong>{{ $product->bv }} BV</strong></span>
                    </div>
                </div>
            </div>
        </div>
...
@endif 
                           <div class="price">
                                <div class="d-flex align-items-end gap-2">
                                    {{-- GUEST PRICE MODIFICATION START --}}
                                    @if (!auth('customer')->check() && isset($product->guest_price) && $product->guest_price > 0 && count(json_decode($product->variation, true)) == 0)
                                        <h3 class="d-flex align-items-end gap-2">
                                            {{ webCurrencyConverter(amount: $product->guest_price - \App\Utils\Helpers::getProductDiscount(product: $product, price: $product->guest_price)) }}
                                        </h3>
                                        @if($product->discount > 0)
                                        <del>
                                            {{ webCurrencyConverter(amount: $product->guest_price) }}
                                        </del>
                                        @endif
                                    @else
                                        <h3 class="d-flex align-items-end gap-2">{!! getPriceRangeWithDiscount(product: $product) !!}</h3>
                                    @endif
                                    {{-- GUEST PRICE MODIFICATION END --}}
                        ...
                        <div class="product-single-pricing">
                            <div class="product-single-pricing-inner text-capitalize">
                                <h2 class="subtitle h6">{{ translate('total_price_for_this_product') }} :</h2>
                                <h3 class="price">
                                <span class="product-details-chosen-price-amount">
                                   
                                    @if(!auth('customer')->check() && isset($product->guest_price) && $product->guest_price > 0 && count(json_decode($product->variation, true)) == 0)
                                        {{webCurrencyConverter($product->guest_price - \App\Utils\Helpers::getProductDiscount(product: $product, price: $product->guest_price))}}
                                    @else
                                        {{webCurrencyConverter($product->unit_price - \App\Utils\Helpers::getProductDiscount(product: $product, price: $product->unit_price))}}
                                    @endif
                                    ...
                                    <ul>
                                        @if ( isset($product['product_type']) && $product['product_type']== 'physical'&& $deliveryInfo['shipping_type'] && $deliveryInfo['shipping_type'] != "order_wise")
                                            <li>
                                                <img loading="lazy"
                                                     ...
                                                     alt="{{ translate('product') }}">
                                                <h4 class="cont h6">
                                                    <div class="t-txt mb-1">{{translate('delivery_charge')}} -</div>
                                        ...
                                        @if(isset($web_config['refund_policy']['status']) && $web_config['refund_policy']['status'] == 1 && $refundDayLimit > 0)
                                            <li>
                                                ...
                                                <h4 class="cont h6">
                                                    <div class="t-txt mb-1">{{ translate('refund_policy') }}-</div>
                                                    <span class="mt-1">{{$refundDayLimit}} {{translate('days')}} <span
                                                            class="mx-1"><a href="{{ route('business-page.view', ['slug' => 'terms-and-conditions']) }}" class="text-base"
    ...
    <section class="recommended-product-section section-gap pb-4">
        <div class="container">
            <div class="section-title mb-4 pb-lg-1">
                <div
    ...
    @include('theme-views.partials.modals._buy-now-referral')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Attach event listener to all social icons
    document.querySelectorAll('.social_share_function').forEach(link => {
        link.addEventListener('click', function () {
            selectedSocial = this.getAttribute('data-social');
            document.getElementById('selectedSocial').value = selectedSocial;
            modal.show();
        });
    });
    
    // Enable/disable share button based on position selection
    document.getElementById('selectPos').addEventListener('change', function() {
        const shareButton = document.getElementById('shareButton');
        shareButton.disabled = (this.value === '');
        
        // If no DSP is selected, show a message or take action
        if (this.value !== '' && document.getElementById('selectDSP').value === '') {
            // You can add any special handling here if needed
            // For now we just proceed with the DDS username automatically
        }
    });
...
function generateShareLink() {
    const social = document.getElementById('selectedSocial').value;
    const selectedDSP = document.getElementById('selectDSP').value;
    const selectedPos = document.getElementById('selectPos').value;
    ...
    switch (social) {
        ...
        case 'twitter':
            ...
            break;
        case 'linkedin':
            baseUrl = 'https://linkedin.com/shareArticle?mini=true&url=';
            break;
        case 'whatsapp':
    ...
    const productUrl = '{{ route('product', ':slug') }}'.replace(':slug', productSlug);
    const refUsername = selectedDSP || username;  // Auto-select DDS if no DSP selected
    
    const fullUrl = `${baseUrl}${encodeURIComponent(`${productUrl}?ref=${refUsername}&pos=${selectedPos}`)}`;
    
    window.open(fullUrl, '_blank', 'width=600,height=400');
    bootstrap.Modal.getInstance(document.getElementById('shareModal')).hide();
    
    // Reset form
    document.getElementById('selectDSP').value = '';
    document.getElementById('selectPos').value = '';
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Normalize and save to localStorage only if both ref and pos exist
        if (ref && pos) {
            // Clean up referralUsername - remove any URL protocols or www
            let cleanRef = ref.trim();
            cleanRef = cleanRef.replace(/^(https?:\/\/)?(www\.)?/, '');
            cleanRef = cleanRef.replace(/\/$/, '');
            
            // Normalize position to 'left' or 'right'
            let cleanPos = pos.trim().toLowerCase();
            if (cleanPos !== 'left' && cleanPos !== 'right') {
                cleanPos = (cleanPos === '1' || cleanPos === 'l') ? 'left' : 'right';
            }
            
            localStorage.setItem('referralUsername', cleanRef);
            localStorage.setItem('referralPosition', cleanPos);
            console.log('Referral saved:', cleanRef, cleanPos);
        }

        // Handle Buy Now button click with referral modal
        const buyNowButtons = document.querySelectorAll('.product-buy-now-button');
        const isLoggedIn = {{ auth('customer')->check() ? 'true' : 'false' }};
        
        // Get admin default referral settings from backend
        ...
        buyNowButtons.forEach(button => {
            ...
            // Capture the original buy now functionality
            const executeBuyNow = function() {
                // Get button data attributes
                const formSelector = button.getAttribute('data-form');
                const isAuth = button.getAttribute('data-auth');
                const cartRoute = button.getAttribute('data-route');
                
                // Find the form and submit it
                const form = document.querySelector(formSelector);
                if (form) {
                    // Add to cart via AJAX (simulating the original behavior)
                    ...
                    .catch(error => {
                        ...
                        // Still redirect to cart on error
                        window.location.href = cartRoute;
                    });
                } else {
            ...
            // Store the function globally so modal can access it
            originalBuyNowAction = executeBuyNow;
            
            // Add our interceptor (use capture phase to run before other handlers)
            button.addEventListener('click', function (e) {
                ...
                // If user is authenticated, bypass modal and execute buy now directly
                if (isLoggedIn) {
                    executeBuyNow();
                    return false;
                }
                ...
                // Check for existing referral in localStorage (PRIORITIZE USER REFERRAL)
                const userReferralUsername = localStorage.getItem('referralUsername');
                const userReferralPosition = localStorage.getItem('referralPosition');
                
                // PRIORITY 1: User has their own referral link (from localStorage)
                if (userReferralUsername && userReferralPosition) {
                    // User referral takes priority - autofill and hide manual entry
                    document.getElementById('referral-username').value = userReferralUsername;
                    document.getElementById('referral-position').value = userReferralPosition;
                    document.getElementById('referral-form-section').style.display = 'none';
                    document.getElementById('autofilled-section').style.display = 'block';
                    
                    // Update the link href with user's referral
                    const registerUrl = '{{ str_replace("://", "://www.", url("/account/user/register")) }}?ref=' + encodeURIComponent(userReferralUsername) + '&position=' + encodeURIComponent(userReferralPosition);
                    document.getElementById('join-autofilled-link').href = registerUrl;
                    
                } 
                // PRIORITY 2: No user referral, but admin has default referral
                else if (adminDefaultReferral.enabled && adminDefaultReferral.username && adminDefaultReferral.position) {
                    // Admin default referral - autofill and hide manual entry
                    document.getElementById('referral-username').value = adminDefaultReferral.username;
                    document.getElementById('referral-position').value = adminDefaultReferral.position;
                    document.getElementById('referral-form-section').style.display = 'none';
                    document.getElementById('autofilled-section').style.display = 'block';
                    
                    // Update the link href with admin's referral
                    const registerUrl = '{{ str_replace("://", "://www.", url("/account/user/register")) }}?ref=' + encodeURIComponent(adminDefaultReferral.username) + '&position=' + encodeURIComponent(adminDefaultReferral.position);
                    document.getElementById('join-autofilled-link').href = registerUrl;
                    
                } 
                // PRIORITY 3: No referral at all - show manual entry
                else {
                    // No referral - show manual entry
                    document.getElementById('referral-username').value = '';
                    document.getElementById('referral-position').value = '';
                    document.getElementById('referral-form-section').style.display = 'block';
                    document.getElementById('autofilled-section').style.display = 'none';
                }
                
                // Show the modal for guest users only
                $('#buyNowReferralModal').modal('show');
                return false;
