...
@section('title', translate('my_inbox') . ' | ' . $web_config['company_name'] . ' ' . translate('ecommerce'))
            ...
            <div class="tab-pane" id="inbox">
                <div class="__chat-area">
                    <div class="__chat-menu">
                        ...
                        <form class="position-relative search--group mb-2" id="chat-search-form">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade show active ">
                                <ul class="nav  nav-tabs-menu overflow-x-hidden flex-row">
                                    @if (isset($allChattingUsers) && count($allChattingUsers) > 0)
                                        @foreach ($allChattingUsers as $key => $chatting)
                                            ...
                                            <div class="nav-item w-100 list_filter">
                                                <a href="javascript:"
                                                    class="chat_list nav-link get-ajax-message-view {{ $loop->iteration == 1 ? 'active' : '' }}"
                                                    data-user-id="{{ $userId }}">
                                                    ...
                                                    <div class="content">
                                                        <div class="d-flex align-items-center">
                                                            <h5 class="name {{ $key != 0 ? 'fw-normal' : '' }}">{{ $userName }}</h5>
                                                            <span class="date">
                                                                {{ $chatting->created_at->diffForHumans() }}
                                                            </span>
                                                        </div>
                                                        <div
                                                            class="d-flex justify-content-between align-items-center msg mt-2">
                                                            @if ($chatting->message)
                                                            ...
                                                            @if ($chatting['unseen_message_count'] > 0)
                                                                <span id="count-unread-messages-{{ $userId }}"
                                                                    class="chatting-count-badge">{{ $chatting['unseen_message_count'] }}</span>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </a>
                                ...
                                <div class="tab-pane fade show active position-relative">
                                    <div class="__chat-content-header">
                                        <a href="javascript:" class="chat-author">
                                            @if ($userType == 'admin')
                                                <img alt="" class="img profile-image" loading="lazy"
                                                    src="{{ getStorageImages(path: $web_config['fav_icon'], type: 'shop') }}">
                                            @elseif($userType == 'vendor')
                                                <img alt="" class="img profile-image" loading="lazy"
                                                    src="{{ getStorageImages(path: $lastChatUser->image_full_url, type: 'shop') }}">
                                            @elseif($userType == 'delivery-man')
                                                <img alt="" class="img profile-image" loading="lazy"
                                                    src="{{ getStorageImages(path: $lastChatUser->image_full_url, type: 'avatar') }}">
                                            @endif
                                    ...
                                    <div
                                        class="temporarily-closed-sticky-alert {{ $userData['temporary-close-status'] == 1 ? '' : 'd-none' }}">
                                        <div class="alert-box">
                                            <div><img src="{{ theme_asset('assets/img/icons/warning.svg') }}"
                                                    alt=""></div>
                                            <div>
                                                {{ translate('sorry') }} !
                                            ...
                                            <div>
                                                <button type="button" class="close close-element-onclick-by-data"
                                                    aria-label="Close" data-selector=".temporarily-closed-sticky-alert">
                                                    <i class="bi bi-x"></i>
                                                </button>
                                            </div>
                                        </div>
                    ...
                    @else
                        <div class="text-center pt-5 flex-grow-1">
                            <div class="text-center mb-5">
                                <img loading="lazy" src="{{ theme_asset('assets/img/icons/nodata.svg') }}"
                                    alt="{{ translate('no_conversation_found') }}">
