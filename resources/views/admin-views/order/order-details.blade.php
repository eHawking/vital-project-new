...
@section('content')
    ...
    <label class="form-label fw-semibold">
        ...
        (
        @if(isset($order->shipping) && is_object($order->shipping))
            {{ translate(str_replace('_', ' ', $order->shipping->title)) }}
        @elseif(isset($order->shipping) && is_string($order->shipping) && !empty($order->shipping))
            {{-- If it's a non-empty string, display it --}}
            {{ translate(str_replace('_', ' ', $order->shipping)) }}
        @else
            {{-- Otherwise, show the default message --}}
            {{ translate('no_shipping_method_selected') }}
        @endif
        )
    </label>
@endif
