...
@section('title', translate('Franchise Orders'))
...
@section('content')
    <div class="content container-fluid">
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
            <h2 class="h1 mb-2 mb-md-0">{{ translate('Franchise Orders') }}</h2>
        </div>
            ...
            <div class="card-header">
                <form action="{{ url()->current() }}" method="GET" class="d-flex flex-grow-1">
                    <div class="input-group">
                        <input type="search" name="search" class="form-control" value="{{ request('search') }}" placeholder="{{ translate('Search by Franchise Name or Order ID') }}">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                        <thead class="thead-light">
                            <tr>
                                <th>#</th>
                                <th>{{ translate('Franchise Name') }}</th>
                        ...
                        <tbody>
                            @forelse ($franchiseOrders as $key => $order)
                                <tr>
                                    <td>{{ $key + $franchiseOrders->firstItem() }}</td>
                                    ...
                                    <td class="text-center">
                                        ...
                                        @if ($order->status == 'pending')
                                            <a href="javascript:void(0)"
                                               ...
                                               onclick="showConfirmationModal('{{ route('admin.franchise.order.approve', ['id' => $order->id]) }}', '{{ translate('Are you sure you want to approve this order?') }}')">
                                               <i class="fas fa-check"></i>
                                            </a>
                                            <a href="javascript:void(0)"
                                               class="btn btn-outline-danger btn-sm"
                                               data-toggle="tooltip" data-placement="top" title="{{ translate('Cancel') }}"
                                               onclick="showConfirmationModal('{{ route('admin.franchise.order.cancel', ['id' => $order->id]) }}', '{{ translate('Are you sure you want to cancel this order?') }}')">
