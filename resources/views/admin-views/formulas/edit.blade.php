...
<div class="content container-fluid">
    ...
    <!-- Form Start -->
    <form action="{{ route('admin.formulas.update', $formula->id) }}" method="POST" id="formula_edit_form">
        @csrf
        @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" name="pv" id="pv" class="form-control" value="{{ old('pv', $formula->pv) }}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="bv">{{ __('BV') }}</label>
                            <input type="number" step="0.01" name="bv" id="bv" class="form-control" value="{{ old('bv', $formula->bv) }}">
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    @foreach ($bonusFields as $field => $label)
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="number" step="0.01" name="{{ $field }}" id="{{ $field }}" class="form-control" value="{{ old($field, $formula->$field) }}">
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" name="promo" id="promo" class="form-control" value="{{ old('promo', $formula->promo) }}">
                        </div>
                        <div class="form-group">
                            <label for="user_promo">{{ __('User Promo') }}</label>
                            <input type="number" step="0.01" name="user_promo" id="user_promo" class="form-control" value="{{ old('user_promo', $formula->user_promo) }}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="seller_promo">{{ __('Seller Promo') }}</label>
                            <input type="number" step="0.01" name="seller_promo" id="seller_promo" class="form-control" value="{{ old('seller_promo', $formula->seller_promo) }}">
                        </div>
                        <div class="form-group">
                            <label for="budget_promo">{{ __('Budget Promo') }}</label>
                            <input type="number" step="0.01" name="budget_promo" id="budget_promo" class="form-control" value="{{ old('budget_promo', $formula->budget_promo) }}">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-3 text-right">
            ...
            <button type="submit" class="btn btn-primary">{{ __('Update Formula') }}</button>
        </div>
    </form>
</div>
@endsection
