            ...
            <table class="border bs-0" style="border-radius:12px;">
                    ...
                    <tr>
                        ...
                        @if($order->billing_address_data)
                            <td colspan="2" class="fs-10 border-left vertical-align-top" style="padding:8px 16px;">
                                ...
                                <div class="">
                                    <div class="font-normal mt-6px text-capitalize">
                                        {{$order->customer?->f_name . ' ' . $order->customer?->l_name}}
                                    </div>
                                    <div class="font-semibold mt-6px text-uppercase">
                                        DDS ID: {{$order->customer?->username ?? 'N/A'}}
                                    </div>
                        ...
                        <td colspan="2" class="fs-10 vertical-align-top  {{$direction === "rtl" ? '' : 'border-left'}}" style="padding:8px 16px;">
                            @if($order->shipping_address_data)
                                ...
                                <div>
                                    <div class="font-normal mt-6px text-capitalize">{{ $order->customer?->f_name . ' ' . $order->customer?->l_name }}</div>
                                    <div class="font-semibold mt-6px text-uppercase">DDS ID: {{$order->customer?->username ?? 'N/A'}}</div>
                ...
                @else
                    <tr>
                        ...
                        <td class="border-bottom" style="padding:23px 8px">
                            ...
                            <div
                                class="font-semibold fs-10 text-capitalize">{{ $order->customer !=null? $order->customer['f_name'].' '.$order->customer['l_name']:translate('Name_not_found')}}</div>
                        </td>
                        <td class="border-bottom" style="padding:23px 8px">
                            @if($order?->customer && $order?->customer?->id != 0)
                                <div class="font-normal mb-1 fs-10">DDS ID</div>
                                <div class="font-semibold fs-10 text-uppercase">
                                    {{ $order->customer?->username ?? 'N/A' }}
                                </div>
                            @elseif($order?->customer == null)
                                <div class="font-normal mb-1 fs-10">DDS ID</div>
                                <div class="font-semibold fs-10">
                                    {{ translate('id_not_found') }}
                                </div>
                            @endif
                        </td>
                        <td class="border-bottom" style="padding:23px 8px">
                ...
                <tr>
                    <td class="customers bs-0 fz-9" colspan="5" style="padding: 10px; width: 100%; border-collapse: collapse">
                        <thead>
                            <tr>
                                <th class="text-uppercase text-dark fz-9 font-normal text-nowrap" style="text-align: {{$direction === "rtl" ? 'end' : 'start'}}">
                            ...
                            @foreach($order->details as $key=>$details)
                                ...
                                <tr>
                                    ...
                                    <td class="text-right text-nowrap">
                                        <div class="fs-10 text-dark">{{ $productDetails->pv ?? 0 }}</div>
                                    </td>
                                    <td class="text-right text-nowrap">
                                        <div class="fs-10 text-dark">{{ $productDetails->bv ?? 0 }}</div>
                                    </td>
                                    <td class="text-right text-nowrap">
                                    ...
                                    $total += $subTotal;
                                    $totalPV += ($productDetails->pv ?? 0) * $details->qty;
                                    $totalBV += ($productDetails->bv ?? 0) * $details->qty;
                                    ?>
                            @endforeach
                        </tbody>
                    </td>
                                ...
                                <th class="calc-table">
                                    <table>
                                        ...
                                        <tr>
                                         ...
                                         <tr>
                                            <td class="text-left font-bold">{{ translate('Total PV')}}</td>
                                            <td class="text-right">{{$totalPV}}</td>
                                        </tr>
                                        <tr>
                                            <td class="text-left font-bold">{{ translate('Total BV')}}</td>
                                            <td class="text-right">{{$totalBV}}</td>
                                        </tr>
