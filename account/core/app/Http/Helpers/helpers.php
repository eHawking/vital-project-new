...
function showAmount($amount, $decimal = 2, $separate = true, $exceptZeros = false, $currencyFormat = true)
{
    ...
    if ($currencyFormat) {
        if (gs('currency_format') == Status::CUR_BOTH) {
            // Remove the currency symbol but keep the currency text
            return $printAmount . ' ' . __(gs('cur_text'));
        } elseif (gs('currency_format') == Status::CUR_TEXT) {
            return $printAmount . ' ' . __(gs('cur_text'));
        } else {
            // Return only the amount if no currency symbol or text is needed
            return $printAmount;
        }
    }
...
function showSingleUserinTree($user)
{
    ...
    // Get the direct referrer
		$refByUser = getUserById($user->ref_by); 
    // Check if the referrer's username matches the 'dsp' pattern
    if ($refByUser && preg_match('/^dsp\d*$/', $refByUser->username)) {
        // Get the referrer's referrer
        $refByRefBy = getUserById($refByUser->ref_by);
		$city = $refByRefBy->city ?? '';
		$name = $user->fullname ?? '';
        $refby = $refByRefBy ? ($refByUser->username . ' of ' . $refByRefBy->username . ' (' . ($refByRefBy->fullname ?? '') . ')') : '';
    } else {
        // If 'dsp' condition is not met or no further referrer exists
		$city = $refByUser->city ?? '';
		$name = $user->fullname ?? '';
    	$refby = $refByUser ? ($refByUser->username . ' (' . ($refByUser->fullname ?? '') . ')') : '';
    }
} else {
    // No direct referrer exists
    $refby = '';
}
	if (preg_match('/^dsp\d*$/', $user->username)) {
		$name = ($refByUser->fullname . "'s DSP") ?? '';
		$mainUserName = $refByUser->username ?? '';
        $planName = $user->username;
		$isDsp = $refByUser->username;
    } else {
		$name = $user->fullname ?? '';
		$mainUserName = $user->username ?? '';
		$planName = $user->dsp_username;
		$isDsp = '';
    }
        ...
        $extraData .= " data-mainusername=\"$mainUserName\""; 
		$extraData .= " data-dspusername=\"$isDsp\"";
        $extraData .= " data-balance=\"" . getAmount($user->balance) . "\""; 
        $extraData .= " data-epincredit=\"" . getAmount($user->epin_credit) . "\""; 
...
function getUserLevel($uplineId, $downlineId)
{
    ...
    $level = 0;
    $currentId = $downlineId;

   
    while($currentId != 0 && $currentId != $uplineId) {
        $level++;
        
        $user = User::find($currentId);

        
        if(!$user || $user->pos_id == 0) {
            return -1;
        }

        $currentId = $user->pos_id;
...
function getUserDepthInTree($userId)
{
    if (!$userId) {
        return 0;
    }
    ...
    $currentPosId = $user->pos_id;

    while ($currentPosId != 0) {
        $level++;
        $parentUser = User::find($currentPosId);

        if (!$parentUser) {
            return -1; 
        }

        $currentPosId = $parentUser->pos_id;
