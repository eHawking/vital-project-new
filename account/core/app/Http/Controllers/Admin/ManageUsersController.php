    ...
    protected function userData($scope = null)
{
    ...
    if (request()->has('search')) {
        ...
        // Apply exact match for username and dsp_username
        $users->where(function ($query) use ($searchTerm) {
            $query->where('username', $searchTerm)
                  ->orWhere('dsp_username', $searchTerm);
        });

        // Apply partial match for other fields
        $users->orWhere(function ($query) use ($searchTerm) {
            $query->where('email', 'like', '%' . $searchTerm . '%')
                  ...
                  ->orWhere('kyc_data', 'like', '%' . $searchTerm . '%');
...
public function update(Request $request, $id)
    {
        ...
        $primaryUser = PrimaryUser::where('username', $user->username)->first();

        if ($primaryUser) {
            $primaryUser->name           = $user->firstname . ' ' . $user->lastname;
            $primaryUser->f_name         = $user->firstname;
            $primaryUser->l_name         = $user->lastname;
            $primaryUser->phone          = $user->mobile;
            $primaryUser->email          = $user->email;
            $primaryUser->street_address = $user->address;
            $primaryUser->country        = $user->country_name;
            $primaryUser->city           = $user->city;

            if ($request->password) {
                $primaryUser->password = Hash::make($request->password);
            }
            $primaryUser->save();
        } else {
            
            $notify[] = ['warning', 'No matching PrimaryUser record found for ' . $user->username . '. PrimaryUser data was not updated.'];
        }
