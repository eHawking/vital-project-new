...
use App\Http\Controllers\Payment_Methods\AlfaPaymentController;
...
use App\Http\Controllers\ReverseSSOController;

Route::get('/transactions', [TransactionController::class, 'index'])->name('orders.index');

// SSO API Routes (for account folder to verify tokens)
Route::prefix('api/sso')->group(function () {
    Route::post('/verify-token', [SSOController::class, 'verifyToken'])->name('api.sso.verify-token');
});

// Reverse SSO Route (for account folder to login user to main script)
Route::get('/sso/reverse-login', [ReverseSSOController::class, 'reverseLogin'])->name('sso.reverse-login');



// Route to sync users
//Route::get('/sync-users', [UserSyncController::class, 'syncUsers']);


Route::post('/customer/transfer-balance-to-wallet', [UserWalletController::class, 'transferBalanceToWallet'])
    ->name('customer.transfer-balance-to-wallet');

// Apply 'auth:customer' and 'checkUserRole' middleware to both routes
Route::middleware(['auth:customer', 'checkUserRole'])->group(function () {
	
	

    // Define the route to display the manual order form
    Route::get('/manual-order', [ManualOrderController::class, 'create'])->name('manual.order.create');

    // Define the route to handle form submission and store the manual order
    Route::post('/manual-order/store', [ManualOrderController::class, 'store'])->name('manual.order.store');
	
	// Route for searching products by name (for the suggestion box)
	Route::get('/ajax/search-products', [ManualOrderController::class, 'searchProducts'])->name('search.products');

	// Route for fetching product details by product ID (when a suggestion is clicked)
	Route::get('/ajax/get-product-details', [ManualOrderController::class, 'getProductDetails'])->name('get.product.details');

	Route::post('/ajax/apply-rules-to-manual-order', [ManualOrderController::class, 'applyRulesToManualOrder'])->name('manual.order.apply-rules');

});


    Route::get('/memberships', [MembershipController::class, 'index'])->name('memberships.index');
    Route::post('/memberships/buy/{id}', [MembershipController::class, 'buy'])->name('memberships.buy');
...
Route::group(['namespace' => 'Web', 'middleware' => ['maintenance_mode', 'guestCheck']], function () {
	...
	Route::get('price-list', [ProductPriceListController::class, 'index'])->name('price-list')->middleware('customer');
    Route::get('/price-list-pdf', [ProductPriceListController::class, 'generatePDF'])
    ->name('price-list.pdf')
    ->middleware('customer');
...
if (!$isGatewayPublished) {
    Route::group(['prefix' => 'payment'], function () {
		...
		//ALFA
        Route::group(['prefix' => 'alfa', 'as' => 'alfa.'], function () {
            Route::get('pay', [AlfaPaymentController::class, 'index'])->name('pay');
            Route::get('success', [AlfaPaymentController::class, 'success'])->name('success');
        });
